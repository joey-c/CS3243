...
class StateTester {
	...
	private StateTester(State s) {
		...
		for (int row = 0; row < existingField.length; row++) {
			System.arraycopy(existingField[row], 0, this.field[row], 0,
					existingField[row].length);
		}
...
public class PlayerSkeleton {
	...
	public int pickMove(State s, int[][] legalMoves) {
		...
		final double[] weights = { -3.261362515614646, 6.842152685545194,
				-4.598013143029638, -11.593996845004808, -5.560643082418299,
				-5.156019621545189, -0.49775723537051064, -12.274343097194112 };
		int moveChoice = 0;
	...
	private double calculateLandingHeightScore(double[] weights,
		...
		double landingHeight = findLandingHeight(stateBeforeMove,
				stateAfterMove);
